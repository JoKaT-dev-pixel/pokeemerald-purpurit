...
static bool8 CanAbilityPreventStatLoss2(u16 abilityDef);
...
static const u16 sNaturePowerMoves[BATTLE_TERRAIN_COUNT] =
{
    ...
    [BATTLE_TERRAIN_BIRCH_LAB]        = MOVE_SWIFT,
};
...
static const u8 sTerrainToType[BATTLE_TERRAIN_COUNT] =
{
    ...
    [BATTLE_TERRAIN_BIRCH_LAB]        = TYPE_NORMAL,
...
static bool32 AccuracyCalcHelper(u16 move)
{
    ...
    || (gStatuses3[gBattlerTarget] & STATUS3_HIDING))
    {
        gMoveResultFlags |= MOVE_RESULT_MISSED;
        JumpIfMoveFailed(7, move);
        return TRUE;
...
static u32 ChangeStatBuffs(s8 statValue, u32 statId, u32 flags, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        else if ((battlerHoldEffect == HOLD_EFFECT_CLEAR_AMULET || CanAbilityPreventStatLoss(battlerAbility) || CanAbilityPreventStatLoss2(battlerAbility))
