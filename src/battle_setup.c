...
{
    CreateMaleMon(&gEnemyParty[0], SPECIES_WHISMUR, 5);
    LockPlayerFieldControls();
...
void BattleSetup_StartTotemBattle(void)
{
    LockPlayerFieldControls();
    gMain.savedCallback = CB2_EndScriptedWildBattle;
    gBattleTypeFlags = BATTLE_TYPE_TOTEM_BATTLE;
    CreateBattleStartTask(GetWildBattleTransition(), 0);
    IncrementGameStat(GAME_STAT_TOTAL_BATTLES);
    IncrementGameStat(GAME_STAT_WILD_BATTLES);
    IncrementDailyWildBattles();
...
u8 BattleSetup_GetTerrainId(void)
{
    ...
    if (MetatileBehavior_IsMudPile(tileBehavior))
        return BATTLE_TERRAIN_MARSH;
    ...
    case MAP_TYPE_BIRCH_LAB:
        return BATTLE_TERRAIN_BIRCH_LAB;
