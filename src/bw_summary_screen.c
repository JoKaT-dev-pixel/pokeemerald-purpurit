...
static EWRAM_DATA struct PokemonSummaryScreenData
{
    ...
    u16 bg1TilemapBuffers[PSS_BUFFER_COUNT][PSS_BUFFER_SIZE];
    u8 mode;
    bool8 isBoxMon;
    u8 curMonIndex;
    u8 maxMonIndex;
...
static const u8 sText_Switch[]                              = _("Changer");
...
static const u8 sText_None[]                                = _("Aucun");
#else
...
static void InitBGs(void)
{
    ...
    SetBgTilemapBuffer(1, sMonSummaryScreen->bg1TilemapBuffers[PSS_BUFFER_BATTLE_EFFECTS]);
...
static bool8 DecompressGraphics(void)
{
    ...
    case 4:
        ...
        break;
    case 5:
        LZDecompressWram(sSummaryEffect_Battle_Tilemap_BW, sMonSummaryScreen->bg1TilemapBuffers[PSS_BUFFER_BATTLE_EFFECTS]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 6:
        LZDecompressWram(sSummaryEffect_Contest_Tilemap_BW, sMonSummaryScreen->bg1TilemapBuffers[PSS_BUFFER_CONTEST_EFFECTS]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 7:
...
static void PssScrollEnd(u8 taskId)
{
    if (sMonSummaryScreen->mode == SUMMARY_MODE_SELECT_MOVE)
    {
        ...
        {
            SetBgTilemapBuffer(1, sMonSummaryScreen->bg1TilemapBuffers[PSS_BUFFER_CONTEST_EFFECTS]);
            ScheduleBgCopyTilemapToVram(1);
...
static void HandleAppealJamTilemap(u16 move)
{
    ...
    // handle appeal
    dst1 = &sMonSummaryScreen->bg1TilemapBuffers[PSS_BUFFER_CONTEST_EFFECTS][TILEMAP_APPEAL_ROW_START];
    ...
    // handle jam
    dst1 = &sMonSummaryScreen->bg1TilemapBuffers[PSS_BUFFER_CONTEST_EFFECTS][TILEMAP_JAM_ROW_1_START];
    dst2 = &sMonSummaryScreen->bg1TilemapBuffers[PSS_BUFFER_CONTEST_EFFECTS][TILEMAP_JAM_ROW_2_START];
    for (i = 0; i < MAX_CONTEST_MOVE_HEARTS; i++)
    {
        if (jam > 0)
        {
...
static void PrintTextOnWindow_BW_Font_Description(u8 windowId, const u8 *string, u8 x, u8 y, u8 lineSpacing, u8 colorId)
{
...
static void PrintContestMoveDescription(u8 moveSlot)
{
    ...
    {
        windowId = AddWindowFromTemplateList(sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_DESCRIPTION);
        FormatTextByWidth(desc, 119, FONT_SMALL_NARROW, gContestEffectDescriptionPointers[gMovesInfo[move].contestEffect], GetFontAttribute(FONT_SMALL_NARROW, FONTATTR_LETTER_SPACING));
        PrintTextOnWindow_BW_Font_Description(windowId, desc, 2, 0, 0, 0);
    }
}
...
static void PrintMoveDetails(u16 move)
{
    ...
    if (move != MOVE_NONE)
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
        {
            ...
            {
                if (gMovesInfo[move].effect != EFFECT_PLACEHOLDER)
                    PrintTextOnWindow_BW_Font_Description(windowId, gMovesInfo[move].description, 2, 0, 0, 0);
                else
                    PrintTextOnWindow_BW_Font_Description(windowId, gNotDoneYetDescription, 2, 0, 0, 0);
            }
        ...
        else
        {
            ...
            {
                PrintTextOnWindow_BW_Font_Description(windowId, gContestEffectDescriptionPointers[gMovesInfo[move].contestEffect], 2, 0, 0, 0);
            }
        }
...
static void PrintHMMovesCantBeForgotten(void)
{
    u8 windowId = AddWindowFromTemplateList(sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_DESCRIPTION);
    FillWindowPixelBuffer(windowId, PIXEL_FILL(0));
    PrintTextOnWindow_BW_Font_Description(windowId, gText_HMMovesCantBeForgotten2, 2, 0, 0, 0);
}
...
{
    if (sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_SHINY] == SPRITE_NONE)
        sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_SHINY] = CreateSprite(&sSpriteTemplate_ShinyIcon, 154, 25, 6);
