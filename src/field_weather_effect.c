...
void Leaves_InitVars(void)
{
    ...
    gWeatherPtr->targetColorMapIndex = 3;
    gWeatherPtr->colorMapStepDelay = 20;
    gWeatherPtr->targetLeavesSpriteCount = NUM_SNOWFLAKE_SPRITES;
    gWeatherPtr->leavesVisibleCounter = 0;
    Weather_SetBlendCoeffs(15, 5); // preserve shadow darkness
    gWeatherPtr->hasShadows = FALSE;
}
...
void Leaves_InitAll(void)
{
    ...
    {
        Leaves_Main();
        for (i = 0; i < gWeatherPtr->leavesSpriteCount; i++)
...
bool8 Leaves_Finish(void)
{
    ...
    {
    case 0:
        gWeatherPtr->targetLeavesSpriteCount = 0;
        gWeatherPtr->leavesVisibleCounter = 0;
        gWeatherPtr->finishStep++;
        // fall through
    case 1:
...
static bool8 UpdateVisibleLeafSprites(void)
{
    if (gWeatherPtr->leavesSpriteCount == gWeatherPtr->targetLeavesSpriteCount)
        return FALSE;

    if (++gWeatherPtr->leavesVisibleCounter > 36)
    {
        gWeatherPtr->leavesVisibleCounter = 0;
        if (gWeatherPtr->leavesSpriteCount < gWeatherPtr->targetLeavesSpriteCount)
            CreateLeafSprite();
    ...
    return gWeatherPtr->leavesSpriteCount != gWeatherPtr->targetLeavesSpriteCount;
}
...
static const union AnimCmd sLeafAnimCmd3[] =
{
    ANIMCMD_FRAME(0, 16),
    ANIMCMD_JUMP(0),
};
...
static bool8 CreateLeafSprite(void)
{
    ...
    gSprites[spriteId].tLeafId = gWeatherPtr->leavesSpriteCount;
    InitLeafSpriteMovement(&gSprites[spriteId]);
    gSprites[spriteId].coordOffsetEnabled = TRUE;
    gWeatherPtr->sprites.s1.rainSprites[gWeatherPtr->leavesSpriteCount++] = &gSprites[spriteId];
    return TRUE;
}

static bool8 DestroyLeafSprite(void)
{
    if (gWeatherPtr->leavesSpriteCount)
    {
        DestroySprite(gWeatherPtr->sprites.s1.rainSprites[--gWeatherPtr->leavesSpriteCount]);
        return TRUE;
    }
...
bool8 Ash_Finish(void)
{
    ...
    {
    case 0:
        Weather_SetTargetBlendCoeffs(0, 10, 1);
        gWeatherPtr->finishStep++;
        break;
    case 1:
        if (Weather_UpdateBlend())
