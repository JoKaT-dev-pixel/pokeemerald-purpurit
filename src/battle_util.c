...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ENDTURN: // 1
        if (gBattleMons[battler].hp != 0)
        {
            ...
            case ABILITY_SOLAR_PANELS:
                if (IsBattlerWeatherAffected(battler, B_WEATHER_SUN)
                 && !BATTLER_MAX_HP(battler)
                 && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK))
                {
                    BattleScriptPushCursorAndCallback(BattleScript_SolarPanelsActivates);
                    gBattleMoveDamage = gBattleMons[battler].maxHP / (gLastUsedAbility == ABILITY_SOLAR_PANELS ? 16 : 8);
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
                    gBattleMoveDamage *= -1;
                    effect++;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_INFLATION:
                ...
                && (CompareStat(battler, STAT_SPATK, MAX_STAT_STAGE, CMP_LESS_THAN)))
            {
                ...
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_TyrantActivates;
                effect++;
            }
            break;
        ...
        case ABILITY_CRYOGENIC:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS
             && IsMoveMakingContact(move, gBattlerAttacker)
             && RandomWeighted(RNG_NITROGEN, 2, 1))
            {
        ...
        case ABILITY_MESMERIZE:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
             && TARGET_TURN_DAMAGED
             && CanBeSlept(gBattlerAttacker, GetBattlerAbility(gBattlerAttacker))
             && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS
             && IsMoveMakingContact(move, gBattlerAttacker)
             && RandomWeighted(RNG_MESMERIZE, 2, 1))
            {
                gBattleScripting.moveEffect = MOVE_EFFECT_AFFECTS_USER | MOVE_EFFECT_SLEEP;
                PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
                BattleScriptPushCursor();
        ...
        case ABILITY_DETERMINATION:
            if (!gSpecialStatuses[gBattlerTarget].switchInAbilityDone
             && !(gBattleStruct->determination[GetBattlerSide(gBattlerTarget)] & gBitTable[gBattlerPartyIndexes[gBattlerTarget]])
             && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
             && TARGET_TURN_DAMAGED
             && (gMultiHitCounter == 0 || gMultiHitCounter == 1) // Activates after all hits from a multi-hit move.
             && IsBattlerAlive(gBattlerTarget)
             && gBattleMons[gBattlerTarget].hp <= (gBattleMons[gBattlerTarget].maxHP / 3)
             && (CompareStat(gBattlerTarget, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN))
             && (CompareStat(gBattlerTarget, STAT_DEF, MAX_STAT_STAGE, CMP_LESS_THAN)))
            {
                gBattlerAttacker = gBattlerTarget;
                gBattleStruct->determination[GetBattlerSide(gBattlerTarget)] |= gBitTable[gBattlerPartyIndexes[gBattlerTarget]];
                gSpecialStatuses[gBattlerTarget].switchInAbilityDone = TRUE;
                gBattleMoveDamage = (GetNonDynamaxMaxHP(gBattlerTarget) / 4) * -1;
                BattleScriptPushCursorAndCallback(BattleScript_DeterminationActivates);
                effect++;
            }
            break;
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_STUN_GUN:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && IsMoveMakingContact(move, gBattlerAttacker)
             && TARGET_TURN_DAMAGED // Need to actually hit the target
             && RandomPercentage(RNG_STUN_GUN, 30))
            {
...
static u8 ItemEffectMoveEnd(u32 battler, u16 holdEffect)
{
    ...
    case HOLD_EFFECT_BERSERK_GENE:
        ...
        effect = ITEM_STATS_CHANGE;
        break;
    case HOLD_EFFECT_COKE_POWDER:
        BufferStatChange(battler, STAT_SPEED, STRINGID_STATROSE);
        gEffectBattler = battler;
        if (CanBeInfinitelyConfused(gEffectBattler))
        {
            gStatuses4[gEffectBattler] |= STATUS4_INFINITE_CONFUSION;
        ...
        BattleScriptPushCursorAndCallback(BattleScript_CokePowderRet);
        effect = ITEM_STATS_CHANGE;
        break;
...
static inline u32 CalcAttackStat(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 isCrit, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk)
{
    ...
    case ABILITY_PERFECTIONNIST:
        if (IS_MOVE_PHYSICAL(move) && IS_MOVE_SPECIAL(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.3));
        break;
    case ABILITY_MATERIALIST:
        if (gBattleMons[battlerAtk].item)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    }
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    else if ((moveType == TYPE_ELECTRIC) && defType == TYPE_GROUND && GetBattlerAbility(battlerAtk) == ABILITY_ZERO_VOLT && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
        if (recordAbilities)
            RecordAbilityBattle(battlerAtk, ABILITY_ZERO_VOLT);
    }
    ...
    if (gMovesInfo[move].effect == EFFECT_EXCALIBUR && defType == TYPE_DRAGON)
        mod = UQ_4_12(2.0);
    if (moveType == TYPE_ROCK && defType == TYPE_FAIRY && IsBattlerGrounded(battlerDef) && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
    if (moveType == TYPE_FIRE && gDisableStructs[battlerDef].tarShot)
        mod = UQ_4_12(2.0);
        
    if (gMovesInfo[move].effect == EFFECT_BOIL_OFF && defType == TYPE_WATER)
        mod = UQ_4_12(2.0);
    if (moveType == TYPE_GRASS && defType == TYPE_ICE && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
    if (moveType == TYPE_FIRE && gDisableStructs[battlerDef].tarShot)
        mod = UQ_4_12(2.0);
