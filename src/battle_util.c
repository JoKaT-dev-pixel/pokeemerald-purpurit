    ...
    case ABILITYEFFECT_MOVES_BLOCK: // 2
        {
            ...
            if ((gLastUsedAbility == ABILITY_SOUNDPROOF && gMovesInfo[move].soundMove && !(moveTarget & MOVE_TARGET_USER))
             || (gLastUsedAbility == ABILITY_AMPLIFIER && gMovesInfo[move].soundMove && !(moveTarget & MOVE_TARGET_USER))
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_NITROGEN:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && CanBePoisoned(gBattlerTarget, gBattlerAttacker)
             && IsMoveMakingContact(move, gBattlerAttacker)
             && RandomWeighted(RNG_COLD_SHOULDER, 2, 1))
            {
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    if (gMovesInfo[move].effect == EFFECT_OIL_BOMB && defType == TYPE_WATER)
        mod = UQ_4_12(2.0);
    if (moveType == TYPE_GRASS && defType == TYPE_BUG && IsBattlerGrounded(battlerDef) && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
    if (moveType == TYPE_FIRE && gDisableStructs[battlerDef].tarShot)
        mod = UQ_4_12(2.0);
