...
static u16 FontFunc_SmallBW(struct TextPrinter *);
...
static u16 FontFunc_Small_Menu(struct TextPrinter *);
...
static void DecompressGlyph_SmallBW(u16, bool32);
...
static void DecompressGlyph_SmallMenu(u16, bool32);
...
static u32 GetGlyphWidth_SmallBW(u16, bool32);
...
static u32 GetGlyphWidth_SmallMenu(u16, bool32);
...
static const struct GlyphWidthFunc sGlyphWidthFuncs[] =
{
    ...
    { FONT_SMALL_BW,     GetGlyphWidth_SmallBW},
    ...
    { FONT_SMALL_MENU, GetGlyphWidth_SmallMenu }
};
...
static const struct FontInfo sFontInfos[] =
{
    ...
    [FONT_SMALL_MENU] = {
        .fontFunction = FontFunc_Small_Menu,
        .maxLetterWidth = 5,
        .maxLetterHeight = 12,
        .letterSpacing = 0,
        .lineSpacing = 0,
...
static u16 FontFunc_SmallBW(struct TextPrinter *textPrinter)
{
    struct TextPrinterSubStruct *subStruct = (struct TextPrinterSubStruct *)(&textPrinter->subStructFields);

    if (subStruct->hasFontIdBeenSet == FALSE)
    {
        subStruct->fontId = FONT_SMALL_BW;
        subStruct->hasFontIdBeenSet = TRUE;
    }
    return RenderText(textPrinter);
}
...
static u16 FontFunc_Small_Menu(struct TextPrinter *textPrinter)
{
    struct TextPrinterSubStruct *subStruct = (struct TextPrinterSubStruct *)(&textPrinter->subStructFields);

    if (subStruct->hasFontIdBeenSet == FALSE)
    {
        subStruct->fontId = FONT_SMALL_MENU;
        subStruct->hasFontIdBeenSet = TRUE;
    }
    return RenderText(textPrinter);
}
...
static u16 RenderText(struct TextPrinter *textPrinter)
{
    ...
    case RENDER_STATE_HANDLE_CHAR:
        ...
        case FONT_BRAILLE:
            break;
        case FONT_SMALL_BW:
            DecompressGlyph_SmallBW(currChar, textPrinter->japanese);
            break;
        case FONT_NARROWER:
        ...
        case FONT_SHORT_NARROW:
            ...
            break;
        case FONT_SMALL_MENU:
            DecompressGlyph_SmallMenu(currChar, textPrinter->japanese);
            break;
        }
...
static void DecompressGlyph_SmallBW(u16 glyphId, bool32 isJapanese)
{
    const u16 *glyphs;
    ...
    {
        glyphs = gFontSmallBWGlyphs + (0x20 * glyphId);
        gCurGlyph.width = gFontSmallBWGlyphWidths[glyphId];
...
static u32 GetGlyphWidth_SmallBW(u16 glyphId, bool32 isJapanese)
{
    if (isJapanese == TRUE)
        return 8;
    else
        return gFontSmallBWGlyphWidths[glyphId];
}
...
static void DecompressGlyph_SmallMenu(u16 glyphId, bool32 isJapanese)
{
    const u16 *glyphs;
    ...
    {
        glyphs = gFontSmallMenuLatinGlyphs + (0x20 * glyphId);
        gCurGlyph.width = gFontSmallMenuLatinGlyphWidths[glyphId];
...
static u32 GetGlyphWidth_SmallMenu(u16 glyphId, bool32 isJapanese)
{
    if (isJapanese == TRUE)
        return 8;
    else
        return gFontSmallMenuLatinGlyphs[glyphId];
}
